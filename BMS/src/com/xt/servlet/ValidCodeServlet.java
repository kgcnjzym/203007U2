 ...
 * @author  杨卫兵
 * @version  [V1.00, 2020-11-26]
 * @since V1.00
 */
public class ValidCodeServlet extends HttpServlet
{
    private String sessionAttrName="code";
    private int length=4;

    @Override
    public void init(ServletConfig config) throws ServletException {
        super.init(config);
        String val=config.getInitParameter("SessionAttrName");
        if(val!=null && ! val.trim().equals("")){
            sessionAttrName=val;
        }
        val=config.getInitParameter("CodeLength");
        try{
            ...
            throws ServletException, IOException
    {
        ...
        se.setAttribute(sessionAttrName, code);
